...
class DecoderLayer(nn.Module):  # self.d_model, MHA1, MHA2, FFN, self.dropout)
    ...
    #     # self.sublayer[1](x, self.feed_forward) # positional feed forward
    #     x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask))  
    #     # x = self.sublayer[1](x, lambda x: self.src_attn(x, memory, memory, src_mask))
    #     return self.sublayer[1](x, self.feed_forward)  # [2, 44, 512]

    def forward(self, x, memory, src_mask, tgt_mask):  # token+tgt, token, token_mask, all_mask
        # self.sublayer[1](x, self.feed_forward) # positional feed forward
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask))  # norm->multi-head-att->add  # to all the first sublayers of decoder
...
class Transformer(nn.Module):   
    def __init__(self, encoder, decoder, src_embed, tgt_embed):
        ...
        # Default: model.forward model.decode model.encode

    def forward(self, src, tgt, src_mask,
                tgt_mask):  # att_feats, seq, att_masks, seq_mask [16, L, 512]  [16, 59]  [16, 1, L]  [16, 59, 59]
    ...
    def decode(self, tgt, memory, src_mask, tgt_mask):
        return self.decoder(torch.cat((memory, self.tgt_embed(tgt)), dim=1), memory, src_mask, tgt_mask)
...
class EncoderDecoder(AttModel):  
    ...
    def make_model(self, tgt_vocab):  
        ...
        model = Transformer(Encoder(EncoderLayer(self.d_model, c(attn), c(ff), self.dropout), self.num_layers_encoder),
            Decoder(DecoderLayer(self.d_model, c(attn), c(attn), c(ff), self.dropout), self.num_layers_decoder),
            lambda x: x,
    ...
    def _prepare_feature_forward(self, att_feats, seq=None):  # [16, L_V_feature, 2048])
        ...
        if seq is not None:
            ...
            mask = att_mask
        return att_feats, seq, att_mask, mask  # memory, target, memory mask, memory + target mask

    def _forward(self,  att_feats, seq):  # only in the forward process of training

        att_feats, seq_target, att_masks, seq_mask = self._prepare_feature_forward(att_feats, seq)
        out = self.model.decode(seq_target, att_feats, att_masks, seq_mask)
        outputs = F.log_softmax(self.logit(out), dim=-1)
        return outputs, self.logit(out)  # outputs.shape([8, 119, 4336]) out.shape([8, 119, 512])
