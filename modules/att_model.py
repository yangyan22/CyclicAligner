...
class AttModel(CaptionModel):
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        memory, seq, memory_masks, all_mask = self._prepare_feature_forward(att_feats)
        # memory, target, memory mask, memory + target mask
        return fc_feats[..., :1], memory[..., :1], memory, memory_masks

    def core(self, it, memory, state):
        if len(state) == 0:
            ys = it.unsqueeze(1)
        else:
            ys = torch.cat([state[0][0], it.unsqueeze(1)], dim=1)

        mask = subsequent_mask(ys.size(1), memory.shape[1]).to(memory.device)
        out = self.model.decode(ys, memory, mask, mask)
        return out[:, -1], [ys.unsqueeze(0)]

    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        # 'it' contains a word index
        output, state = self.core(it, p_att_feats, state)  # output == current probability, state = generated word ids == progressively concate the generated words
        logprobs = F.log_softmax(self.logit(output), dim=1)
        return logprobs, state

    def _sample(self, fc_feats, att_feats, att_masks=None):  # only in sampling process of training and testing
        ...
        if beam_size > 1 and sample_method == "beam_search":
            return self._sample_beam(fc_feats, att_feats, att_masks, opt)

        elif group_size > 1 and sample_method == "group":
            ...
            trigrams = []
            seq = fc_feats.new_full((batch_size * sample_n, self.max_seq_length), self.pad_idx, dtype=torch.long)  # BS, max_length     All zeros
            seqLogprobs = fc_feats.new_zeros(batch_size * sample_n, self.max_seq_length, self.vocab_size + 1)  # BS, max_length, all vocab+1 (special) All zeros

            for t in range(self.max_seq_length + 1):
                if t == 0:  # input <bos>
                    it = fc_feats.new_full([batch_size * sample_n], self.bos_idx, dtype=torch.long)  # [BS] all bos_idx==0
                # print(output_logsoftmax) # whether to output the probabilities
                ...
                if unfinished.sum() == 0:
                    break
        return seq, seqLogprobs  # max_lenth=60 containing max_lenth words (exclude the bos)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        return seq, seqLogprobs  # seq shape: BS, max_length; seqLogprobs shape: BS, max_length, vocab_size+1
